...
public class Exercise11Test {
    ...
    public void checkFileContent() {
        Path outputFile = Paths.get("output/books_filtered.txt");
        assertTrue("File books_filtered.txt does not exist or cannot be accessed!", outputFile != null && Files.isRegularFile(outputFile) && Files.isReadable(outputFile));
        Path inputFile = Paths.get("input/books_filtered.txt");
