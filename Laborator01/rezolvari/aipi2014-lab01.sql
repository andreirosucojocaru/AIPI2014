...
SELECT b.id, 
	   b.title, 
       (SELECT GROUP_CONCAT(DISTINCT CONCAT(w.first_name, ' ', w.last_name)) 
FROM author a, writer w 
WHERE a.book_id=b.id AND w.id=a.writer_id) AS author FROM book b 
ORDER BY b.id;

# EXERCISE 4
UPDATE book SET price = price * 0.2 WHERE printing_year < 2010;

SELECT printing_year, title, price FROM book ORDER BY printing_year ASC, price DESC;

# EXERCISE 5
SELECT CONCAT(w.first_name, ' ', w.last_name) AS writer,
       (SELECT GROUP_CONCAT(b.title) FROM book b, author a WHERE b.id=a.book_id AND a.writer_id = w.id) AS books,
...
# EXERCISE 6
DELIMITER //
CREATE FUNCTION calculate_supply_order_value1 (
...
DELIMITER //
CREATE FUNCTION calculate_invoice_value1 (
	identification_number VARCHAR(20)
)
RETURNS DECIMAL(8,2)
BEGIN
...
SELECT identification_number, calculate_supply_order_value1(identification_number) AS supply_order_value FROM supply_order ORDER BY supply_order_value DESC LIMIT 3;

SELECT identification_number, calculate_invoice_value1(identification_number) AS invoice_value FROM invoice ORDER BY invoice_value DESC LIMIT 3;

# EXERCISE 7
DELIMITER //
CREATE PROCEDURE calculate_user_total_invoice_value1 (
	IN personal_identifier BIGINT(13),
	OUT total_invoice_value DECIMAL(8,2)
)
BEGIN
	SELECT SUM(calculate_invoice_value1(identification_number)) INTO total_invoice_value FROM invoice i WHERE i.user_personal_identifier = personal_identifier;
	SELECT identification_number, calculate_invoice_value1(identification_number), issue_date FROM invoice i WHERE i.user_personal_identifier = personal_identifier ORDER BY issue_date ASC;
	SELECT CONCAT('Valoare Totala : ',total_invoice_value,' RON'); 
END; //

CALL calculate_user_total_invoice_value1('1290621115475',@user_total_invoice_value);
SELECT @user_total_invoice_value;

# EXERCISE 8
SELECT b.id, 
       b.title, 
	   (SELECT GROUP_CONCAT(CONCAT(w.first_name, ' ', w.last_name)) FROM author a, writer w WHERE a.book_id = b.id AND w.id = a.writer_id) AS authors, 
       SUM(id.quantity * b.price) AS total_sell 
FROM book b, invoice_detail id  
WHERE id.book_id = b.id 
GROUP BY b.id 
...
# EXERCISE 9
DELIMITER //
CREATE TRIGGER book_insert_check BEFORE INSERT ON book FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.edition <= 0 THEN
        SET message = concat('Editia unei carti trebuie sa fie o valoare strict pozitiva ', cast(NEW.edition AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
    END IF;  
   IF NEW.stockpile < 0 THEN
        SET message = concat('Stocul unei carti trebuie sa fie o valoare pozitiva ', cast(NEW.stockpile AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF; 
    IF NEW.price <= 0 THEN
        SET message = concat('Pretul unei carti trebuie sa fie o valoare strict pozitiva ', cast(NEW.price AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF; 
END; //

DELIMITER //
CREATE TRIGGER book_update_check BEFORE UPDATE ON book FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.edition <= 0 THEN
        SET message = concat('Editia unei carti trebuie sa fie o valoare strict pozitiva ', cast(NEW.edition AS CHAR));
...
DELIMITER //
CREATE TRIGGER supply_order_insert_check BEFORE INSERT ON supply_order FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.state NOT IN ('sent', 'delivered') THEN
        SET message = concat('Starea unei comenzi poate fi doar sent sau delivered ', cast(NEW.state AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
END; //

DELIMITER //
CREATE TRIGGER supply_order_update_check BEFORE UPDATE ON supply_order FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.state NOT IN ('sent', 'delivered') THEN
        SET message = concat('Starea unei comenzi poate fi doar sent sau delivered ', cast(NEW.state AS CHAR));
...
DELIMITER //
CREATE TRIGGER invoice_insert_check BEFORE INSERT ON invoice FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.state NOT IN ('issued', 'paid') THEN
        SET message = concat('Starea unei facturi poate fi doar issued sau paid ', cast(NEW.state AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
END; //

DELIMITER //
CREATE TRIGGER invoice_update_check BEFORE UPDATE ON invoice FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.state NOT IN ('issued', 'paid') THEN
        SET message = concat('Starea unei facturi poate fi doar issued sau paid ', cast(NEW.state AS CHAR));
...
DELIMITER //
CREATE TRIGGER supply_order_detail_insert_check BEFORE INSERT ON supply_order_detail FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.quantity < 0 THEN
        SET message = concat('Cantitatea unei comenzi trebuie sa fie o valoare strict pozitiva ', cast(NEW.quantity AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
END; //

CREATE TRIGGER supply_order_detail_update_check BEFORE UPDATE ON supply_order_detail FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.quantity < 0 THEN
        SET message = concat('Cantitatea unei comenzi trebuie sa fie o valoare strict pozitiva ', cast(NEW.quantity AS CHAR));
...
DELIMITER //
CREATE TRIGGER user_insert_check BEFORE INSERT ON user FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.email NOT LIKE '%@%.%' THEN
        SET message = concat('Emailul trebuie sa aiba forma user@domain.country ', cast(NEW.email AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
    IF NEW.type NOT IN ('administrator', 'client') THEN
        SET message = concat('Tipul trebuie sa aiba valoarea administrator sau client ', cast(NEW.type AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
    IF NEW.role NOT IN ('super-administrator', 'regular administrator', 'registered client', 'verified client') THEN
        SET message = concat('Rolul trebuie sa aiba una din valorile: super-administrator, regular administrator, registered client, verified client ', cast(NEW.role AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
END; //

DELIMITER //
CREATE TRIGGER user_update_check BEFORE UPDATE ON user FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.email NOT LIKE '%@%.%' THEN
        SET message = concat('Emailul trebuie sa aiba forma user@domain.country ', cast(NEW.email AS CHAR));
...
DELIMITER //
CREATE TRIGGER invoice_detail_insert_check BEFORE INSERT ON invoice_detail FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.quantity < 0 THEN
        SET message = concat('Cantitatea unei facturi trebuie sa fie o valoare strict pozitiva ', cast(NEW.quantity AS CHAR));
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = message;
     END IF;  
END; //

CREATE TRIGGER invoice_detail_update_check BEFORE UPDATE ON invoice_detail FOR EACH ROW
BEGIN
    DECLARE message VARCHAR(255);
    IF NEW.quantity < 0 THEN
        SET message = concat('Cantitatea unei comenzi trebuie sa fie o valoare strict pozitiva ', cast(NEW.quantity AS CHAR));
...
#EXERCISE 10
DELIMITER //
CREATE PROCEDURE mean_age_by_gender() 
BEGIN 
    DECLARE done INT DEFAULT FALSE; 
    DECLARE number_personal_identifier BIGINT(13);
    DECLARE string_personal_identifier VARCHAR(13);
    DECLARE birth_year, birth_month, birth_day VARCHAR(4);
    DECLARE men_total_age, women_total_age, men_number, women_number, current_age BIGINT; 
    DECLARE present_date, birth_date DATE; 
    DECLARE user_cursor CURSOR FOR SELECT personal_identifier FROM user; 
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    
    SET men_total_age := 0;
    SET women_total_age := 0;
    SET men_number := 0;
    SET women_number := 0;
    SET present_date := CURRENT_DATE;
    
    OPEN user_cursor; 
    
	my_loop: LOOP FETCH user_cursor INTO number_personal_identifier; 
			 IF done 
                 THEN LEAVE my_loop; 
             END IF; 
             SET string_personal_identifier := CAST(number_personal_identifier AS CHAR);
             
			 SET birth_year := SUBSTR(string_personal_identifier, 2, 2);
             IF CAST(birth_year AS DECIMAL) > 14 THEN 
                 SET birth_year := CONCAT('19',birth_year);
             ELSE 
                 SET birth_year := CONCAT ('20',birth_year);
             END IF;
             SET birth_month := SUBSTR(string_personal_identifier, 4, 2);
             SET birth_day := SUBSTR(string_personal_identifier, 6, 2);
             
		     SET birth_date := CAST(CONCAT(birth_year,'-',birth_month,'-',birth_day) AS DATE);
             
             SET current_age := DATEDIFF(present_date, birth_date) / 365;
             
			 IF LEFT(string_personal_identifier, 1) = '1' THEN 
                 SET men_number := men_number + 1; 
                 SET men_total_age := men_total_age + current_age;
			 ELSEIF LEFT(string_personal_identifier,1 ) = '2' THEN 
                 SET women_number := women_number +1; 
                 SET women_total_age := women_total_age + current_age;
             END IF; 
    END LOOP; 
    SELECT CONCAT('Varsta medie a barbatilor este ', men_total_age / men_number,'. Varsta medie a femeilor este ',women_total_age / women_number,'.');
	CLOSE user_cursor; 
END; //

CALL mean_age_by_gender;
