...
public class ContactsList {
    ...
    public ContactsList(String userName) {
        ...
        currentMessagingWindows = new HashMap<>();
    }
    ...
    public void start() {
        ...
        currentContactsListItem = new TreeItem<>(Constants.CONTACTS_LIST, currentContactListIcon);
        ...
        connectedContactsListItem = new TreeItem<>(Constants.CONNECTED_CONTACTS_LIST, connectedContactListIcon);
        ...
        recentContactsListItem = new TreeItem<>(Constants.RECENT_CONTACTS_LIST, recentContactListIcon);
    ...
    public void handleCurrentMessagingWindows(String interlocutorUserName, MessageExchange messageExchange) {
        ...
        if (!found) {
            recentContactsListItem.getChildren().add(new TreeItem<>(interlocutorUserName));
            writeToFile();
        }
    }
	...
	// TO DO (exercise 6): analyze login messages and update
        ...
        if (messageContent != null && messageContent.equals(Constants.LOGIN_MESSAGE)) {
            ...
            if (!found) {
                connectedContactsListItem.getChildren().add(new TreeItem<>(interlocutorUserName));
            }
            return;
        }
    ...
    private void readFromFile() {
        ...
        try (RandomAccessFile file = new RandomAccessFile(Constants.TEMPORARY_DIRECTORY + currentUserName, "rw")) {
            ...
            while ((contactUserName = file.readLine()) != null) {
                recentContactsListItem.getChildren().add(new TreeItem<>(contactUserName));
            }
            file.close();
