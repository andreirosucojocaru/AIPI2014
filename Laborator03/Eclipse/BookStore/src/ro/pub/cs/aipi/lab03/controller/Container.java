...
public class Container implements EventHandler<Event> {
    ...
    private void initialize() {
        try {
            ...
            tableNames.stream().map((tableName) -> new MenuItem(Utilities.tableNameToMenuEntry(tableName))).map((menuItem) -> {
        ...
        databaseManagementTabPane.getSelectionModel().selectedItemProperty().addListener(
                (ObservableValue<? extends Tab> observableValue, Tab oldTab, Tab newTab) -> {
    ...
    protected void aboutMenuItemHandler(ActionEvent event) {
        ...
        dialog.setProperties(Constants.ABOUT_WINDOW_TITLE,
                Constants.ABOUT_ICON_LOCATION,
                Constants.ABOUT_MESSAGE_CONTENT);
        dialog.start();
    }
