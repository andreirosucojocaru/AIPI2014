...
import ro.pub.cs.aipi.lab03.dataaccess.DataBaseException;
...
public class TableManagement {
    ...
    private void updateButtonHandler(MouseEvent mouseEvent) {
        ...
        ArrayList<String> tableViewCurrentRecord = ((Model) tableContentTableView.getSelectionModel().getSelectedItem()).getValues();
        int currentIndex = 0;
        ...
        try {
            if (attributes != null && values != null && !attributes.isEmpty() && !values.isEmpty()) {
                databaseWrapper.updateRecordsIntoTable(tableName,
                        attributes,
                        values,
                        databaseWrapper.getTablePrimaryKey(tableName) + "=\'" + tableViewCurrentRecord.get(0) + "\'");
            }
        } catch (SQLException | DataBaseException sqlException) {
            System.out.println("An exception had occured: " + sqlException.getMessage());
            if (Constants.DEBUG) {
    ...
    private void searchButtonHandler(MouseEvent mouseEvent) {
        ...
        for (Control attributeControl : attributesControls) {
            ...
            if (value != null && !value.isEmpty() && currentIndex != 0) {
                whereClause += ((whereClause.isEmpty()) ? "" : " AND ") + attributesLabels.get(currentIndex).getText() + " LIKE \'%" + value + "%\'";
            }
            currentIndex++;
        }
    ...
    private void clearButtonHandler(MouseEvent mouseEvent) {
        ...
        for (Control attributeControl : attributesControls) {
            if (attributeControl instanceof ComboBox) {
                ...
                try {
                    if (currentIndex == 0 && databaseWrapper.isAutoGeneratedPrimaryKey(tableName)) {
                        ((TextField) attributeControl).setText(Integer.toString(databaseWrapper.getTablePrimaryKeyMaximumValue(tableName) + 1));
                    } else {
